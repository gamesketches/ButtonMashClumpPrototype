...
public class PlayerMovement : MonoBehaviour {
	...
	public void HandleRotation() {
		...
		reticle.transform.localPosition = new Vector3(Mathf.Cos((currentAngle + 180.0f * player.number) * Mathf.Deg2Rad) * reticleRadius, Mathf.Sin((currentAngle + 180.0f * player.number) * Mathf.Deg2Rad) * reticleRadius, 0.0f);
		reticle.transform.localRotation = Quaternion.Euler(0.0f, 0.0f, (currentAngle + 180.0f * player.number) - 90.0f);
		reticleBlast.transform.localPosition = new Vector3(Mathf.Cos((currentAngle + 180.0f * player.number) * Mathf.Deg2Rad) * reticleBlastRadius, Mathf.Sin((currentAngle + 180.0f * player.number) * Mathf.Deg2Rad) * reticleBlastRadius, 0.0f);
		reticleBlast.transform.localRotation = reticle.transform.localRotation;
	}
	...
	public float currentShotAngle() {
		if(useBenAiming) {
			return currentAngle + 180.0f * player.number;
		} else {
